...
const HomeView: React.FC<HomeViewProps> = ({ onSelectDiscipline }) => {
  const activeDisciplines = SIMULATIONS.filter(s => s.status === 'active' || s.status === 'coming-soon');
  const otherDisciplines = SIMULATIONS.filter(s => s.status === 'locked');
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <section className="bg-[#003366] p-10 rounded-[2.5rem] shadow-2xl mt-20 text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h4 className="font-black text-2xl mb-4 tracking-tight uppercase italic">Medicina <span className="text-[#D4A017]">do Sertão</span></h4>
          <p className="text-blue-100 text-sm leading-relaxed max-w-3xl font-light">
            O projeto <strong>Medicina do Sertão</strong> foca na <strong>prática deliberada</strong>. Recomendamos que você utilize os simulados como ferramenta de diagnóstico: identifique suas lacunas de conhecimento e use os <strong>resumos compartilhados</strong> para reforçar o aprendizado colaborativo.
          </p>
        </div>
      </section>
    </main>
